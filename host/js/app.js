...
function switchtheme(a){if(a=='a'){setCookie('theme_mode','a',365);}else if(a=='d'){setCookie('theme_mode','d',365);}else if(a=='l'){setCookie('theme_mode','l',365);}else if(a=='s'){if(getCookie("theme_mode")=="a"){setCookie("theme_mode", "d", 365);}else if(getCookie("theme_mode")=="d"){setCookie("theme_mode", "l", 365);}else if(getCookie("theme_mode")=="l"){setCookie("theme_mode", "a", 365);}}else{cl('Error: Invalid theme mode');}changetheme();}function theme(){if(checkCookie('theme_mode')==true){if(getCookie('theme_mode')=='a'){setCookie('theme','a',365);}else if(getCookie('theme_mode')=='d'){setCookie('theme','d',365);}else if(getCookie('theme_mode')=='l'){setCookie('theme','l',365);}}else{if(typeof prefered_theme_mode!='undefined'){if(prefered_theme_mode=='a' && prefered_theme_mode=='d' && prefered_theme_mode=='l'){switchtheme(prefered_theme_mode);}else{switchtheme('a');}}else{switchtheme('a');}theme();}}function changetheme(){if(getCookie('theme')=='a'){if(gebi('themec').content=='web'){gebi('stylesheet').innerHTML='@media(prefers-color-scheme:dark){'+webdarkcss+'}@media(prefers-color-scheme:light){'+weblightcss+'}'+webcss;}else if(gebi('themec').content=='app'){gebi('stylesheet').innerHTML='@media(prefers-color-scheme:dark){'+appdarkcss+'}@media(prefers-color-scheme:light){'+applightcss+'}'+appcss;}else if(gebi('themec').content=='acc'){gebi('stylesheet').innerHTML='@media(prefers-color-scheme:dark){'+accdarkcss+'}@media(prefers-color-scheme:light){'+acclightcss+'}'+acccss;}}else if(getCookie('theme')=='d'){if(gebi('themec').content=='web'){gebi('stylesheet').innerHTML=webcss+webdarkcss;}else if(gebi('themec').content=='app'){gebi('stylesheet').innerHTML=appcss+appdarkcss;}else if(gebi('themec').content=='acc'){gebi('stylesheet').innerHTML=acccss+accdarkcss;}}else if(getCookie('theme')=='l'){if(gebi('themec').content=='web'){gebi('stylesheet').innerHTML=webcss+weblightcss;}else if(gebi('themec').content=='app'){gebi('stylesheet').innerHTML=appcss+applightcss;}else if(gebi('themec').content=='acc'){gebi('stylesheet').innerHTML=acccss+acclightcss;}}}
...
function getprehtml(a){
  console.log(a);
  const xhr1=new XMLHttpRequest(); const xhr2=new XMLHttpRequest();
  xhr1.open('GET','/api/prepage/meta/'+a);        xhr2.open('GET','/api/prepage/html/'+a);
  xhr1.send();                                          xhr2.send();
...
function route(){
  ...
  if(udata.urlpna[0]=='signup' || udata.urlpna[0]=='signin' || udata.urlpna[0]=='register' || udata.urlpna[0]=='login' || udata.urlpna[0]=='forgot'){
    ...
    } else if(udata.urlpna[0]=='signin' || udata.urlpna[0]=='login'){
      getprehtml('signin');
    }
