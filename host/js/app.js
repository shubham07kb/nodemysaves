function cl(a){console.log(a)}function gebi(a){return document.getElementById(a);}function gesbcn(a){return document.getElementsByClassName(a);}function setls(a,b){localStorage.setItem(a,b);}function getls(a){return localStorage.getItem(a);}function removels(a){localStorage.removeItem(a);}function clearls(){localStorage.clear();;}function jwthandler(a){try{p=jwt_decode(a)}catch(e){p=undefined};return p;}
...
function accounthandler(){
  if(checkCookie('_acc_header') || checkCookie('_acc_data') || checkCookie('_acc_key') || checkCookie('_s_acc_header') || checkCookie('_s_acc_data') || checkCookie('_s_acc_key') || checkCookie('_c_acc_header') || checkCookie('_c_acc_data') || checkCookie('_c_acc_key')){
    if(checkCookie('_acc_header') && checkCookie('_acc_data') && checkCookie('_acc_key') &&  checkCookie('_s_acc_header') && checkCookie('_s_acc_data') && checkCookie('_s_acc_key') && checkCookie('_c_acc_header') && checkCookie('_c_acc_data') && checkCookie('_c_acc_key')){
      a=jwthandler(getCookie('_acc_header')+'.'+getCookie('_acc_data')+'.'+getCookie('_acc_key'));
      b=jwthandler(getCookie('_s_acc_header')+'.'+getCookie('_s_acc_data')+'.'+getCookie('_s_acc_key'));
      c=jwthandler(getCookie('_c_acc_header')+'.'+getCookie('_c_acc_data')+'.'+getCookie('_c_acc_key'));
      if(a==null || b==null || c==null){
        removeAccountAtError();
      } else if(a.id==b.id && b.id==c.id && a.id==c.id && b.email==c.email && b.username==c.username && b.role==c.role && b.fname==c.fname && b.lname==c.lname){
        globalacc=c;
      } else{
        removeAccountAtError();
      }
...
function removeAccountAtError(){
  userrmkey();
}
async function userrmkey(){
  if(checkCookie('rmkey')){
    p=await fetch('/mo');
    console.log(p);
  } else{
    globalacc={role:'g'};
...
function app(){httpscheck();ipform();accounthandler();socketconnect();presetup();theme();tools();route();endloader();}
